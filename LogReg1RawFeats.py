...
def gradDec(theta,data,labels):
    ...
    grad = np.dot(error, dataAdd1) / labels.size
    return grad
...
trainData1 = np.loadtxt('synthetic-1.csv', dtype='float', delimiter=',')
...
plt.figure(1)
plt.title("Gradient Descent w/ Raw Features: Synthetic-1")
...
p = predict(thetaFinal,dataAdd1,labels)
